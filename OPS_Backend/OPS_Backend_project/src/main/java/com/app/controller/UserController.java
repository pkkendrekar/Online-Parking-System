...
@CrossOrigin(origins = "http://localhost:3000")//npm start default 3000
...
public class UserController {
	...
	@GetMapping(value="/findParking/{searchParking}")
	public responseDTO<?> findParkingByCity(@PathVariable String searchParking) {
		
		
		return new responseDTO<>(HttpStatus.OK, "Displaying Parking list from given city",
				userService.findParkingByCity(searchParking));
	}
	
	@PostMapping("/bookParking")
	public responseDTO<?> BookParking(@RequestBody bookParkingDTO park){
		
		
		return new responseDTO<>(HttpStatus.OK, "Booking Done",
				userService.BookParking(park));
	}
	
	@GetMapping("/view_bookingHistoryByCusId/{userId}")
	public responseDTO<?> viewbookingHistoryByCusId(@PathVariable int userId) {
		return new responseDTO<>(HttpStatus.OK, "Viewcustomer by cusID",
				userService.viewbookingHistoryByCusId(userId));}
	
	@PostMapping("/payment/{bookid}")
	public responseDTO<?> updateStatus(@PathVariable int bookid) {
		return new responseDTO<>(HttpStatus.OK, "payment status updated",
				userService.updateStatus(bookid));}
